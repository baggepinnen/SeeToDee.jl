...
- [`SeeToDee.ETDRK2`](@ref) A 2nd-order exponential Runge-Kutta integrator for semilinear stiff systems ``\dot{x} = Lx + N(x,u,p,t)``. Integrates the linear part exactly via the matrix exponential; no stability restriction from ``L``. If called with StaticArrays, this method is allocation free.
- [`SeeToDee.ETDRK3`](@ref) A 3rd-order exponential Runge-Kutta integrator (Krogstad 2005) for semilinear stiff systems. Same stability properties as ETDRK2 with higher accuracy. If called with StaticArrays, this method is allocation free.
- [`SeeToDee.ETDRK4`](@ref) A 4th-order exponential Runge-Kutta integrator (Cox & Matthews 2002) for semilinear stiff systems. Reduces to classical RK4 as ``L \to 0``. If called with StaticArrays, this method is allocation free.
